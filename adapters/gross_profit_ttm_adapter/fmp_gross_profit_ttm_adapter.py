# adapters/gross_profit_ttm_adapter/fmp_gross_profit_ttm_adapter.py
...
class FMPGrossProfitTTMAdapter(MetricAdapter):
    """
    Computes Gross Profit TTM by summing the last 4 quarterly gross profit values via FMP.
    ...
    def __init__(self) -> None:
        self._name = "fmp_gross_profit_ttm"
    ...
    def fetch(self, ticker: str) -> float:
        ...
        try:
            ...
            count = 0
            for row in data:
                val = _f(row.get("grossProfit")) or _f(row.get("Gross Profit")) or _f(row.get("gross_profit"))
                if val is not None:
                    total += val
                    count += 1

            if count == 0:
                raise DataNotAvailable(f"{self._name}: no quarterly gross profit values for {ticker}")
