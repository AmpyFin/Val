# adapters/tickers_adapter/wiki_sp600_tickers_adapter.py
...
class WikiSP600TickersAdapter(TickersAdapter):
    """
    Scrapes S&P 600 (SmallCap) constituents from Wikipedia and returns a list of ticker symbols.
    ...
    def fetch(self) -> List[str]:
        try:
            resp = requests.get(WIKI_SP600_URL, timeout=HTTP_TIMEOUT, headers=HEADERS)
            ...
            raise DataNotAvailable(f"{self._name}: failed to fetch/parse S&P 600 tickers") from exc
