# adapters/tickers_adapter/wiki_sp400_tickers_adapter.py
...
class WikiSP400TickersAdapter(TickersAdapter):
    """
    Scrapes S&P 400 (MidCap) constituents from Wikipedia and returns a list of ticker symbols.
    ...
    def fetch(self) -> List[str]:
        try:
            resp = requests.get(WIKI_SP400_URL, timeout=HTTP_TIMEOUT, headers=HEADERS)
            ...
            raise DataNotAvailable(f"{self._name}: failed to fetch/parse S&P 400 tickers") from exc
